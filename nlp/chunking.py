...
class LangchainSplitterClient:
    ...
    def _get_tokenizer(self):
        if LangchainSplitterClient._tokenizer is None:
            ...
            if not complete_dir_and_files:
                logger.info(f"Load tokenizer from remote: {self.model_checkpoint}...")
                ...
                logger.info(f"Load tokenizer from local directory: {self.model_dir}")
                try:
                    LangchainSplitterClient._tokenizer = AutoTokenizer.from_pretrained(
                        str(self.model_dir)
                    )
            ...
            logger.info("Tokenizer has been loaded successfully.")
    ...
    def chunk_texts(self, texts, max_tokens):
        ...
        splitter = RecursiveCharacterTextSplitter.from_huggingface_tokenizer(
            tokenizer, chunk_size=max_tokens, chunk_overlap=50
        )
...
class StatisticalChunkerClient:
    ...
    def _get_encoder(self):
        if StatisticalChunkerClient._encoder is None:
            ...
            if not complete_dir_and_files:
                logger.info(f"Load encoder from remote: {self.model_checkpoint}...")
                ...
                logger.info(f"Load encoder from local directory: {self.model_dir}")
                try:
                    StatisticalChunkerClient._tokenizer = HuggingFaceEncoder(
            ...
            logger.info("Encoder has been loaded successfully.")
