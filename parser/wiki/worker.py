    ...
    for attempt in range(max_retries):
        try:
            ...
            if attempt == max_retries - 1:
                logger.error(f"Worker failed to connect to Weaviate after {max_retries} attempts: {e}")
                raise
...
def process_batch_wrapper(task_args):
    ...
    return process_batch(
        batch_data, batch_idx, expected_total_batches, time_start,
        _worker_mongo_client, _worker_weaviate_client
    )